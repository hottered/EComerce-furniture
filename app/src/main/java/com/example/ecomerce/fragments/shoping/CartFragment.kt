...
class CartFragment : Fragment(R.layout.fragment_cart) {
    ...
    private val viewMode by activityViewModels<CartViewModel>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            viewMode.productsPrice.collectLatest { price ->
                price?.let {
                    totalPrice = it
        ...
        cartAdapter.OnPlusClick = {
            viewMode.changeQuantity(it,FirebaseCommon.QuantityChanging.INCREASE)
        }
        ...
        lifecycleScope.launchWhenStarted { 
            viewMode.deleteDialog.collectLatest { 
                val alertDialog = AlertDialog.Builder(requireContext()).apply { 
                    setTitle("Delete item form cart ?")
                    setMessage("Do  you want to delete item from your cart ?")
                    setNegativeButton("Cancel") { dialog,_->
                        dialog.dismiss()
                    }
                    setPositiveButton("Yes") { dialog,_ -> 
                        viewMode.deleteCartProduct(it)
                        dialog.dismiss()
                    }
                }
                alertDialog.create()
